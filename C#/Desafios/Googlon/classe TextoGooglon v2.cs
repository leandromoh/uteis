...
{
    public class StringT : IComparable<StringT>
    {
        public static char[] ordemAlfabetica = { 'j', 'n', 'g', 'm', 'c', 'l', 'q', 's', 'k', 'r', 'z', 'f', 'v', 'b', 'w', 'p', 'x', 'd', 'h', 't' };
        public static Regex regexPreposicao = new Regex("^[ac-z]{2}[vshz]$");
        public static Regex regexVerbo = new Regex("^[a-z]{6,}[bvshz]$");
        public static Regex regexVerboPrimeiraPessoa = new Regex("^[^bvshz][a-z]{5,}[bvshz]$");
        public string value;

        public StringT(string Value)
        {
            value = Value;
        }

		public static Func<long, bool> isBeautifulNumber = num => num >= 787808 && num % 3 == 0;
        ...
        public int CompareTo(StringT other) => Compare(value, other.value);
        ...
        public static int Compare(string a, string b)
        {
            ...
            if (emptyA || emptyB)
                return emptyA ^ emptyB ? !emptyA ? 1 : -1 : 0;

            int posA = Array.IndexOf(ordemAlfabetica, a[0]);
            int posB = Array.IndexOf(ordemAlfabetica, b[0]);

            return (posA > posB) ? 1 : (posA < posB) ? -1 : Compare(a.Substring(1), b.Substring(1));
        }
    ...
    {
        private List<StringT> palavras;

        public TextoMadegues(string Texto)
        {
            palavras = Texto.Split(' ').Select(s => new StringT(s)).ToList();
        }
        ...
        public int ContaNumerosBonitos() => palavras.Count(s => StringT.isBeautifulNumber(s.toNumber()));

        public string PegarListaDeVocabulario()
        {
            var distintas = palavras.Distinct().ToList();
            distintas.Sort();
            return String.Join(" ", distintas.Select(s => s.value));
        }
