...
namespace ConsoleApplication2
{
    class Program
    {
        ...
        static Queue<Caminho> estadosParaExplorar = new Queue<Caminho>();
        ...
        {
            criarConexoes();
            estadosParaExplorar.Enqueue(new Caminho("A", 0));

            while (estadosParaExplorar.Count != 0)
            {
                var conexao = estadosParaExplorar.Dequeue();
                ...
                foreach (var c in conexoesPossiveis)
                {
                    ...
                    if (!existe(conexaoPossivel))
                        estadosParaExplorar.Enqueue(conexaoPossivel);
                }
            }
